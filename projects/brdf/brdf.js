...
function initGradients() {
	for(var i = 0; i< 256; i++) {
		gradients.push({x: Math.cos(i * 2.0 * Math.PI / 256), y: Math.sin(i * 2.0 * Math.PI / 256)});
	}
}
...
function setGradients() {
	try {
		var s = shaders[currentShader].program;
		for (i = 0; i < 256; i++) {
			var grad = new Float32Array([gradients[i].x, gradients[i].y]);
			gl.uniform2fv(s.gradientsUniform[i], grad);
		}
	} catch(e) {}
}
...
function locateAttribsAndUniforms(shaderName, shaderProgram) {
	...
	// create uniform gradients

		shaderProgram.gradientsUniform = new Array();
		for (i = 0; i < 256; i++) {
			shaderProgram.gradientsUniform[i] = gl.getUniformLocation(shaderProgram, "gradients[" + i + "]");
		}
...
function toggleMaterial() {
	...
	var msg = "Current Material: " + materialNames[currentMaterial];
	showMessage(msg);
	updateOptionControls();
}
