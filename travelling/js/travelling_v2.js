    ...
    tracks.forEach((track) => {
      ...
      switch (track.type) {
        case "flight": {
          ...
          geometry.paths = [
            fromLatLng.slice().reverse(),
            toLatLng.slice().reverse(),
          ];
          symbol.color = COLORS["flight"];

          distanceData[track.type].dist +=
            geolib.getDistance(
        ...
        default: {
          ...
          const line = coordinates.map(([latitude, longitude]) => ({
            latitude,
            longitude,
          }));
          distanceData[track.type].dist += geolib.getPathLength(line) / 1000;
          distanceData[track.type].times += 1;
          break;
        }
      }
    ...
    showStats([
      {
        ...
        values: Object.entries(distanceData).map(([type, value]) => ({
          type,
          ...value,
        })),
      },
    ...
    worldJson.features.forEach((country) => {
      if (Object.keys(citiesPerCountry).includes(country.properties.name)) {
        const symbol = {
          ...
          color: [
            57,
            200 - (200 * citiesPerCountry[country.properties.name]) / maxVisits,
            200,
            0.8,
          ],
          outline: {
